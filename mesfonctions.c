...
int tirerCarteC(CChance** listeCartesChance, int* nbCartes){
    CChance tmp;
    ...
    if(strcmp(listeCartesChance[0]->nom,"Carte sortez de prison")==0){
        *nbCartes -= 1;
        for(int i=0;i<*nbCartes;i++){
            strcpy(listeCartesChance[i]->nom,listeCartesChance[i+1]->nom);
        }
        return 1;
    }
    else{
        CChance* temp = listeCartesChance[0];
        for(int i=0;i<*nbCartes;i++){
            strcpy(listeCartesChance[i]->nom,listeCartesChance[i+1]->nom);
        }
        strcpy(listeCartesChance[*nbCartes-1]->nom,temp->nom);
        return 0;
    }
}

int tirerCarteComm(CCommunaute ** listeCartesComm, int* nbCartes){
    CChance tmp;
    printf("Vous avez tirez la carte :\n"
           "\t==> %s",listeCartesComm[0]->nom);
    if(strcmp(listeCartesComm[0]->nom,"Carte sortez de prison")==0){
        *nbCartes -= 1;
        for(int i=0;i<*nbCartes;i++){
            strcpy(listeCartesComm[i]->nom,listeCartesComm[i+1]->nom);
        }
        return 1;
    }
    else{
        CChance* temp = listeCartesComm[0];
        for(int i=0;i<*nbCartes;i++){
            strcpy(listeCartesComm[i]->nom,listeCartesComm[i+1]->nom);
        }
        strcpy(listeCartesComm[*nbCartes-1]->nom,temp->nom);
        return 0;
    }
}

void ajouterFinDeListe(CChance** listeCartes, int* nbCartes, const char* nom){
    if(*nbCartes == 10){
        for(int i=0;i<*nbCartes;i++){
                    ...
                    if(listeJoueurs[i].nbCSP!=0){
                        ...
                        switch(choix){
                            case 1:
                                ...
                                if(*nbCarteC==9){
                                    strcpy(listeCartesChance[9].nom,"Carte sortez de prison");
                                    break;
                                }
                                else{
                                    strcpy(listeCartesComm[9].nom,"Carte sortez de prison");
                                    break;
                                }
                            case 2:
                                break;
                        ...
                        for(int j=0;j<*nbJoueurs;j++){
                            if(listeJoueurs[j].nbCSP > 1){
                                ...
                                switch(choix){
                                    case 1:
                                        printf("%s, %s souhaite vous acheter votre carte sortie de prison pour xxx euros, acceptez vous ?\n");
                                        break;
                                    case 2:
                                        break;
                                    default:
...
//Cases tirage carte communauté.
                            ...
                            listeJoueurs[i].nbCSP += tirerCarteComm(listeCartesComm,&nbCarteComm);
...
//Cases tirage carte chance.
                            ...
                            listeJoueurs[i].nbCSP += tirerCarteC(listeCartesChance,&nbCarteC);
